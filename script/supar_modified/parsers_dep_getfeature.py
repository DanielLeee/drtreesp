...
class BiaffineDependencyParser(Parser):
    ...
    def _evaluate(self, loader):
        ...
        for batch in loader:
            ...
            s_arc, s_rel, s_random_feature = self.model(words, feats)
            # yeshu's code end
            ...
            # yeshu's code begin
            for di in range(words.shape[0]):
                cur_data = {}
                cur_data['length'] = ori_mask[di].sum().cpu().item() + 1
                cur_data['sentence'] = ' '.join(texts[di])
                # cur_data['feature'] = s_random_feature[di, :cur_data['length'], :cur_data['length']].permute(1, 0, 2).cpu().numpy()
                cur_data['feature'] = s_random_feature[di, :, :cur_data['length'], :].cpu().numpy()
                cur_data['groundtruth'] = torch.stack((arcs[di], rels[di]), dim = 1)[ori_mask[di]].cpu().numpy()
                cur_data['punctuation_mask'] = mask[di][ori_mask[di]].cpu().numpy()
                cur_data['relation_prediction_all_arcs'] = s_rel[di][:cur_data['length'], :cur_data['length']].argmax(-1).cpu().numpy().T
                processed_data_list.append(cur_data)
            # yeshu's code end
        ...
        # yeshu's code begin
        return total_loss, metric, processed_data_list
        # yeshu's code end
...
class CRF2oDependencyParser(BiaffineDependencyParser):
    ...
    def _evaluate(self, loader):
        ...
        for batch in loader:
            ...
            # yeshu's code begin
            ori_mask = mask.detach().clone()
            s_arc, s_sib, s_rel, s_random_feature = self.model(words, feats)
            # yeshu's code end
            ...
            for di in range(words.shape[0]):
                ...
                cur_data['length'] = ori_mask[di].sum().cpu().item() + 1
                cur_data['sentence'] = ' '.join(texts[di])
                # cur_data['feature'] = s_random_feature[di, :cur_data['length'], :cur_data['length']].permute(1, 0, 2).numpy()
                cur_data['feature'] = s_random_feature[di, :, :cur_data['length'], :].cpu().numpy()
                cur_data['groundtruth'] = torch.stack((arcs[di], rels[di]), dim = 1)[ori_mask[di]].cpu().numpy()
                cur_data['punctuation_mask'] = mask[di][ori_mask[di]].cpu().numpy()
                cur_data['relation_prediction_all_arcs'] = s_rel[di][:cur_data['length'], :cur_data['length']].argmax(-1).cpu().numpy().T
