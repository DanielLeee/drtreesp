...
class Parser(object):
    ...
    def train(self, train, dev, test, buckets=32, batch_size=5000, update_steps=1,
              clip=5.0, epochs=1000, patience=100, **kwargs):
        ...
        # yeshu's code begin
        self.model.arc_attn.reset_parameters()
        for n, p in self.model.named_parameters():
            p.requires_grad = n.startswith('arc_attn')
        # yeshu's code end

        if args.encoder == 'lstm':
            # yeshu's code begin
            self.optimizer = Adam(filter(lambda p : p.requires_grad, self.model.parameters()), args.lr, (args.mu, args.nu), args.eps, args.weight_decay)
            # yeshu's code end
            # original code begin
    ...
    def evaluate(self, data, buckets=8, batch_size=5000, **kwargs):
        ...
        # yeshu's code begin
        loss, metric, processed_data_list = self._evaluate(dataset.loader)
        # yeshu's code end
        ...
        # yeshu's code begin
        return loss, metric, processed_data_list
        # yeshu's code end
        return loss, metric
    ...
    def load(cls, path, reload=False, src='github', checkpoint=False, **kwargs):
        ...
        parser.checkpoint_state_dict = state['checkpoint_state_dict'] if args.checkpoint and 'checkpoint_state_dict' in state else None
        return parser
