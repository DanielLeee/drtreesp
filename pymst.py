...
def get_mst_np(np_arr, one_root = True):
    ...
    n = np_arr.shape[0]
    w_simple = np_arr.min(axis = 2)
    w_pos = np_arr.argmin(axis = 2)
    ret_tree = fast_parse(-w_simple, one_root = one_root)
    ret_mat = np.zeros(np_arr.shape)
    pred_rels = w_pos[ret_tree[1:], np.arange(1, n)]
    ret_mat[ret_tree[1:], np.arange(1, n), pred_rels] = 1
