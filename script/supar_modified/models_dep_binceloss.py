...
class BiaffineDependencyModel(Model):
    ...
    def loss(self, s_arc, s_rel, arcs, rels, mask, partial=False):
        ...
        # arc_preds, _ = self.decode(s_arc, s_rel, mask, tree = True)
        # arc_preds = arc_preds[mask]

        s_arc, arcs = s_arc[mask], arcs[mask]
        arc_gt = torch.zeros(s_arc.shape).to(device)
        arc_gt[torch.arange(len(arcs)), arcs] = 1

        # arc_margin = (s_arc[torch.arange(len(arcs)), arc_preds] - s_arc[torch.arange(len(arcs)), arcs]).mean()

        s_rel, rels = s_rel[mask], rels[mask]
        s_rel = s_rel[torch.arange(len(arcs)), arcs]
        rel_gt = torch.zeros(s_rel.shape).to(device)
        rel_gt[torch.arange(len(rels)), rels] = 1

        # rel_margin = (s_rel.max(-1).values - s_rel[torch.arange(len(rels)), rels]).mean()

        # arc_loss = self.criterion(s_arc, arcs)
        # rel_loss = self.criterion(s_rel, rels)
        arc_loss = self.bce(s_arc, arc_gt)
        rel_loss = self.bce(s_rel, rel_gt)
